...
export default function Rotation({ selectedAircraft, selectedDate }: Props) {
  ...
  const loadRotation = () => {
    ...
    setRotation(all[selectedAircraft]?.[selectedDate] || []);
  };
  ...
  const saveRotation = (newRotation: Rotation[]) => {
    ...
    const stored = localStorage.getItem("rotation-by-aircraft");
    const all = stored ? JSON.parse(stored) : {};
    if (!all[selectedAircraft]) all[selectedAircraft] = {};
    all[selectedAircraft][selectedDate] = newRotation;
